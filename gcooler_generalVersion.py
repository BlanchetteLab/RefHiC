...
class bandmatrix():
    def __init__(self, pixels, extent, max_distance_bins=None, bins=None, info=None):
        ...
        self.diag_mean = np.mean(self.bmatrix, axis=0)
        self.bins = bins
        self.bp2bin = \
    ...
    def rows(self, firstRow, lastRow, type='o', returnBias=False):
        ...
        if returnBias:
            ...
            # p2ll = self.p2ll(output[0,:,:],cw=3) # prefer to use obs to compuate p2ll
            return outRows, bias
    ...
    def __squareFromContinousRows(self, xCenter, yCenter, w, type='o', meta=True):
        ...
        if meta:
            ...
            p2ll = self.p2ll(output[0, :, :], cw=3)  # prefer to use obs to compuate p2ll
            return output, np.concatenate((bias, [self.totalRC, p2ll]))
        return output
    ...
    def p2ll(self, x, cw=3):
        ...
        if sum(llcorner) == 0:
            return 0
        return x[c, c] / (sum(llcorner) / len(llcorner))
