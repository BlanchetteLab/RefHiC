...
def trainAttention(prefix,lr,fr,elr,elr_lambda,elr_beta,name,batchsize, epochs, gpu, trainingset, skipchrom, resol, n, test, extra, bedpe, max_distance,w,oversampling,feature,ti,encoding_dim,models,eval_ti,coteach):
    ...
    if trainingset.endswith('.h5') or trainingset.endswith('.hdf5'):
        ...
        for i in range(trainingset_H5.attrs['num']):
            ...
            if value[1] != _skipchrom:
                training_index[value[-1]].append(value[0])
        ...
        print('load data')
        for idx in tqdm(y_idx):
            idx = str(idx)
            # if ti is None:
            x = grp[idx]['X'][:, featureMask]
            # else:
            #     x = grp[idx]['X'][ti, featureMask][None]
            xs = grp[idx]['Xs'][:, featureMask]
            X.append(x)
            Xs.append(xs)
        with open(trainingset+'.pkl', 'wb') as handle:
            pickle.dump((X,Xs,y_label),handle, protocol=pickle.HIGHEST_PROTOCOL)

    elif trainingset.endswith('.pkl'):
        print('reading pkl')
        with open(trainingset, 'rb') as handle:
            X, Xs, y_label = pickle.load(handle)

    if eval_ti is None:
        eval_ti = {}
        for _i in range(X[0].shape[0]):
    ...
    y_label_train, y_label_test, X_train, X_test, Xs_train, Xs_test = train_test_split(y_label, X, Xs, test_size = 0.1, random_state = 42, shuffle=True)

    # _flipidx=np.random.choice(len(y_label_train), len(y_label_train)//10, replace=False)
    # y_label_train[_flipidx] = 1-y_label_train[_flipidx]
    ...
    for _k in eval_ti:
        ...
        test_dataloaders[_k] = DataLoader(test_data, batch_size=batchsize, shuffle=True,num_workers=1)
    ...
    if 'grouploop' in models.lower():
        if coteach:
            ...
            for epoch in tqdm(range(0, epochs)):
                ...
                testModel(model,test_dataloaders, criterion,device,epoch,TBWriter=TBWriter)
                # scheduler.step()
                # scheduler2.step()
            ...
            for epoch in tqdm(range(0, epochs)):
                if elr:
                    ...
                    testModel(model, test_dataloaders, criterion, device, epoch, TBWriter=TBWriter,elr=True,printData= (epoch == epochs-1))
                else:
                    trainLoss=trainModel(model,train_dataloader, optimizer, criterion, epoch, device,batchsize,TBWriter=TBWriter,elr=False)
                    testLoss=testModel(model,test_dataloaders, criterion,device,epoch,TBWriter=TBWriter,elr=False,printData= (epoch == epochs-1))
    ...
    if 'baseline' in models.lower():
        ...
        for epoch in tqdm(range(epochs)):
            ...
            testLoss=testModel(baselineModel,test_dataloaders, criterion,device,epoch,TBWriter=TBWriterB,baseline=True)
            # schedulerB.step()
            if testLoss < earlyStopping['loss'] and earlyStopping['wait'] < earlyStopping['patience']:
                earlyStopping['loss'] = testLoss
                earlyStopping['epoch'] = epoch
