...
def pred2(batchsize, gpu, chrom, n, input, reference, max_distance,modelstate,output,alpha):
    ...
    for _chrom in chrom:
        ...
        with torch.no_grad():
            ...
            for X in tqdm(test_dataloader):
                ...
                # target
                pred = torch.sigmoid(model(targetX[...,featureMask].to(device),X[1][...,featureMask].to(device))).cpu()
                ...
                #decoy
                pred = torch.sigmoid(model(decoyX[..., featureMask].to(device), X[1][..., featureMask].to(device))).cpu()
                predLs = pred[:, 0].numpy().tolist()
                predRs = pred[:, 1].numpy().tolist()
                start = X[2][0].cpu().numpy().flatten().tolist()
                end = (X[2][0].cpu().numpy().flatten() + parameters['resol']).tolist()
    ...
    # data=data[data[3]>0.5].reset_index(drop=True)


    decoyTADB  = data[(data[7]=='decoy') & (data[3]>0.5)]
    print(decoyTADB)
    n=0
    pileup = np.zeros((2 * 20 + 1, 2 * 20 + 1))
    for x,y in zip(list(decoyTADB[1]),list(decoyTADB[2])):
        mat,meta= bmatrix[1].square(x,y,20,'oe')
        pileup+=mat[0,:,:]
        n+=1
    pileup/=n
