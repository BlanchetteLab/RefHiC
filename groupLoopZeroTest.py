...
import h5py
from groupLoopModels import groupLoopZero
import numpy as np
...
@click.option('--celltypeEmbedding',type=str,required=True,help='celltype embeddings')
def groupLoopZeroTest(batchsize, gpu, trainingset, skipchrom, resol, n, test, extra, bedpe, max_distance,w,oversampling,feature,ti,encoding_dim,eval_ti,savedmodel,celltypeembedding):
    if gpu is not None:
    ...
    with open(celltypeembedding,'rb') as f:
        celltypeembedding = pickle.load(f)

    model = groupLoopZero(np.sum(featureMask), encoding_dim=encoding_dim,testCelltype=celltypeembedding['test'],extraCelltypes=celltypeembedding['extra'],batchsize=batchsize,device=device).to(device)
    ...
    with torch.no_grad():
        for key in test_dataloaders:
            ...
            for X in tqdm(test_dataloaders[key]):
                ...
                output = torch.sigmoid(model(X[1].to(device), X[2].to(device)))
