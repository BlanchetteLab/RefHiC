...
def trainAttention(prefix, trainingset, skipchrom, resol, test, extra, bedpe, max_distance,w,oversampling,feature,ti,eval_ti):
    ...
    y = y_label_train + y_label_val

    print(X[0].shape)
    for i in range(len(X)):
        x=X[i]
        x=np.concatenate([x[:(2*w+1)**2], stats.rankdata(x[:(2*w+1)**2], method='ordinal'),x[[-1]]], axis=-1)
        X[i] = x
    ...
    RFModel = trainRF(X,y,nproc=10)

    dump(RFModel, prefix+'.joblib')
